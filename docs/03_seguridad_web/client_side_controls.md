---
titulo: "Controles client-side y validacion del servidor"
...
fuentes_internas:
  - ../../tools/concepts.py#client-side-controls
fuentes_externas:
...
# Controles client-side y validacion del servidor
...
JavaScript, campos hidden, botones deshabilitados, validaciones HTML y datos almacenados en el navegador mejoran la interfaz, pero el cliente controla la peticion final. El servidor debe repetir validacion, autenticacion y autorizacion sobre cada operacion.

Burp o curl permiten omitir la interfaz y cambiar parametros, metodos, cookies y cuerpos. Si el backend confia en role=admin, price, user_id o una comprobacion hecha solo en JavaScript, el control se puede saltar.
...
Cuando la interfaz oculta acciones, limita valores o decide permisos antes de enviar una peticion al backend.
...
Captura una peticion valida, cambia una sola propiedad controlada por el cliente y compara la respuesta con otra cuenta o rol.

Evidencia esperada: Evidencia reproducible de que el servidor acepta o rechaza correctamente el cambio, separada del comportamiento visual del navegador.
...
```text
curl -i "$URL/profile?user_id=1001" -H "Cookie: session=$COOKIE"
```

**Objetivo y contexto:** Reproduce directamente una peticion autenticada para comprobar si el servidor valida la propiedad del recurso.

**Resultado esperado:** Un 403 confirma control; datos de otra cuenta confirman un fallo de autorizacion.

## Anatomía de los payloads

La primera prueba es `curl -i "$URL/profile?user_id=1001" -H "Cookie: session=$COOKIE"`. Separa sus delimitadores, operadores, opciones, operandos y variables; algunos elementos no estarán presentes según el contexto. Las comillas, barras y separadores pertenecen a una capa concreta. Usa la explicación de cada capa para determinar qué símbolo altera sintaxis y cuál transporta datos.
...
Evidencia reproducible de que el servidor acepta o rechaza correctamente el cambio, separada del comportamiento visual del navegador.
...
- Observa la peticion real.
- Identifica el dato que impone la UI.
- Repite en Repeater.
- Cambia solo ese dato.
- Compara por cuenta, status y cuerpo.

Amplía únicamente dentro del laboratorio y cuando cada salto aporte una capacidad nueva demostrable.

## Errores frecuentes

- Cambiar la interfaz sin conseguir una respuesta distinta del servidor no demuestra impacto.
- Un 403 consistente al repetir la peticion indica que el backend si aplica el control.
...
Todo valor controlado por el navegador puede modificarse; la seguridad real debe imponerse en el servidor. La técnica queda confirmada solo cuando una prueba mínima produce la evidencia prevista y descarta explicaciones alternativas.
...
- [Concepto fuente de THM Fieldbook](../../tools/concepts.py) (`client-side-controls`)
