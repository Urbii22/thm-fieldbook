...
XML permite definir 'entidades' (variables de texto) en un DOCTYPE, incluidas entidades que cargan contenido EXTERNO (un fichero local o una URL). Si la app procesa XML que tu controlas y el parser no desactiva esas entidades externas, defines una que apunte a /etc/passwd y el parser te devuelve su contenido.

Muchos parsers XML traen la resolucion de entidades externas activada POR DEFECTO por compatibilidad, y el programador no la desactiva explicitamente. El parser confia en que el XML que procesa es de confianza, pero si TU controlas el XML de entrada, controlas sus entidades.
...
Solo aplica cuando la app procesa XML que tu controlas: subida de ficheros XML/SVG/DOCX/XLSX, SOAP, SAML, o cualquier import que acepte XML. No aplica si la app solo usa JSON.
...
Sube un XML con una entidad que apunte a un fichero que sepas que existe (file:///etc/passwd) y mira si el contenido vuelve reflejado.

Evidencia esperada: El contenido de /etc/passwd aparece en la respuesta (XXE 'visible'). Si no se refleja nada, intenta un XXE ciego (out-of-band) apuntando la entidad a tu propio listener HTTP.
...
**Objetivo y contexto:** Construye el XML minimo: declara una entidad externa 'xxe' que apunta a un fichero local, y la referencia dentro del documento.

**Resultado esperado:** El fichero xxe.xml listo para enviar. El contenido de /etc/passwd deberia sustituir a &xxe; si el parser resuelve la entidad.
...
**Objetivo y contexto:** Envia el XML malicioso al endpoint que lo procesa. El Content-Type application/xml asegura que el servidor lo trate como XML y no como texto plano.

**Resultado esperado:** Si el parser es vulnerable, la respuesta contiene las lineas de /etc/passwd (root:x:0:0...) en vez de procesar el documento normalmente.

## Anatomía de los payloads

La primera prueba es `printf '<?xml version="1.0"?><!DOCTYPE root [<!ENTITY xxe SYSTEM "file:///etc/passwd">]><root>&xxe;</root>' > xxe.xml`. Separa sus delimitadores, operadores, opciones, operandos y variables; algunos elementos no estarán presentes según el contexto. Las comillas, barras y separadores pertenecen a una capa concreta. Usa la explicación de cada capa para determinar qué símbolo altera sintaxis y cuál transporta datos.
...
El contenido de /etc/passwd aparece en la respuesta (XXE 'visible'). Si no se refleja nada, intenta un XXE ciego (out-of-band) apuntando la entidad a tu propio listener HTTP.
...
Un parser XML mal configurado resuelve entidades externas que tu defines, dejandote leer ficheros del servidor o hacer peticiones salientes. La técnica queda confirmada solo cuando una prueba mínima produce la evidencia prevista y descarta explicaciones alternativas.
...
- [Concepto fuente de THM Fieldbook](../../tools/concepts.py) (`xxe`)
