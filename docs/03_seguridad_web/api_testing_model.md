---
titulo: "Modelo de pruebas de API"
...
fuentes_internas:
  - ../../tools/concepts.py#api-testing-model
fuentes_externas:
...
Una API expone contratos HTTP que deben probarse por recurso, metodo, entrada y contexto de identidad.

Un inventario incompleto hace que se pierdan endpoints administrativos, versiones antiguas o metodos menos visibles.
...
Cuando la aplicacion usa JSON, rutas /api, documentacion OpenAPI, GraphQL o clientes SPA.
...
Identifica al menos un endpoint funcional, su metodo, parametros y una respuesta reproducible.

Evidencia esperada: Dispones de un mapa de recursos y de una lista priorizada para probar autenticacion y autorizacion.
...
**Objetivo y contexto:** Busca un contrato OpenAPI publicado.

**Resultado esperado:** Listado de rutas, metodos y esquemas.
...
Dispones de un mapa de recursos y de una lista priorizada para probar autenticacion y autorizacion.
...
- Descubre rutas y metodos.
- Repite peticiones con entradas controladas.
- Anota diferencias de estado y permisos.

Amplía únicamente dentro del laboratorio y cuando cada salto aporte una capacidad nueva demostrable.

## Errores frecuentes

- No aporta mucho si solo existe una pagina estatica sin peticiones de datos ni autenticacion.
...
Mapear endpoints, metodos, parametros y respuestas antes de intentar explotar una API. La técnica queda confirmada solo cuando una prueba mínima produce la evidencia prevista y descarta explicaciones alternativas.
...
- [Concepto fuente de THM Fieldbook](../../tools/concepts.py) (`api-testing-model`)
