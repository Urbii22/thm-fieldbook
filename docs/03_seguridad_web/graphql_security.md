...
fuentes_internas:
  - ../../tools/concepts.py#graphql-security
fuentes_externas:
...
GraphQL concentra muchos recursos en un endpoint y permite seleccionar campos mediante un schema.

Introspection abierta, resolveres sin autorizacion y consultas costosas amplian la superficie de ataque.
...
Cuando observas /graphql, content-type application/graphql o cuerpos JSON con query.
...
Una peticion valida devuelve datos o errores GraphQL estructurados.

Evidencia esperada: Tienes el schema disponible o una lista de operaciones y sabes donde probar permisos y limites.
...
```text
curl -sS "$URL/graphql" -H 'Content-Type: application/json' --data '{"query":"{ __typename }"}'
```

**Objetivo y contexto:** Confirma que el endpoint procesa GraphQL.

**Resultado esperado:** Respuesta data o error GraphQL.

### Capa 2: prueba documentada

```text
curl -sS "$URL/graphql" -H 'Content-Type: application/json' --data @introspection.json
```

**Objetivo y contexto:** Solicita el schema si esta permitido.

**Resultado esperado:** Tipos, campos y operaciones disponibles.

## Anatomía de los payloads

La primera prueba es `curl -sS "$URL/graphql" -H 'Content-Type: application/json' --data '{"query":"{ __typename }"}'`. Separa sus delimitadores, operadores, opciones, operandos y variables; algunos elementos no estarán presentes según el contexto. Las comillas, barras y separadores pertenecen a una capa concreta. Usa la explicación de cada capa para determinar qué símbolo altera sintaxis y cuál transporta datos.
...
Tienes el schema disponible o una lista de operaciones y sabes donde probar permisos y limites.
...
- Comprueba introspection.
- Enumera queries y mutations.
- Repite operaciones con identidades distintas.

Amplía únicamente dentro del laboratorio y cuando cada salto aporte una capacidad nueva demostrable.

## Errores frecuentes

- No aplica a APIs REST sin endpoint GraphQL identificable.
...
Enumerar schema, queries, mutations y controles de autorizacion en GraphQL. La técnica queda confirmada solo cuando una prueba mínima produce la evidencia prevista y descarta explicaciones alternativas.
...
- [Concepto fuente de THM Fieldbook](../../tools/concepts.py) (`graphql-security`)
