...
WordPress es el CMS mas comun en rooms. Su nucleo se actualiza solito, asi que rara vez es el vector. El fallo vive en los plugins y temas de terceros (miles, muchos sin mantener) o en credenciales debiles del panel /wp-admin, desde donde se llega a ejecucion editando un tema.

Cada plugin es codigo de terceros con su propia superficie; una version vulnerable de un plugin popular es explotable en masa. Y el panel de admin permite editar los .php del tema: si entras, tienes RCE casi directo. wpscan automatiza detectar version de core, plugins, temas y usuarios.
...
Cuando whatweb o /wp-login.php delatan WordPress. Encaja tras el fingerprint web, antes de intentar bugs genericos.
...
wpscan --url $URL --enumerate vp,u y revisa si algun plugin sale marcado [!] vulnerable.

Evidencia esperada: Version del core, lista de plugins con su version, y usuarios enumerados via /?author= o REST API.
...
```text
wpscan --url $URL --enumerate vp,u --plugins-detection aggressive
```

**Objetivo y contexto:** Enumera plugins vulnerables (vp) y usuarios (u). La deteccion agresiva encuentra plugins que la pasiva no ve. Es el primer paso siempre en WordPress.

**Resultado esperado:** Version del core, plugins con su version y [!] de vulnerabilidad, y la lista de usuarios. Cada plugin vulnerable -> busca su exploit; cada usuario -> candidato a login.

## Anatomía de los payloads

La primera prueba es `wpscan --url $URL --enumerate vp,u --plugins-detection aggressive`. Separa sus delimitadores, operadores, opciones, operandos y variables; algunos elementos no estarán presentes según el contexto. Las comillas, barras y separadores pertenecen a una capa concreta. Usa la explicación de cada capa para determinar qué símbolo altera sintaxis y cuál transporta datos.
...
Version del core, lista de plugins con su version, y usuarios enumerados via /?author= o REST API.
...
El core suele estar parcheado; el agujero casi siempre esta en un plugin o tema vulnerable, o en el panel de admin. La técnica queda confirmada solo cuando una prueba mínima produce la evidencia prevista y descarta explicaciones alternativas.
...
- [Concepto fuente de THM Fieldbook](../../tools/concepts.py) (`wordpress`)
