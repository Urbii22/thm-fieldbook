...
fuentes_internas:
  - ../../tools/concepts.py#dll-hijacking
fuentes_externas:
...
Cuando un programa carga una DLL sin ruta absoluta, Windows la busca por un orden de directorios. Si un proceso privilegiado busca una DLL que no existe (o busca antes en un directorio donde puedes escribir), colocas ahi una DLL maliciosa con ese nombre y el proceso la carga y ejecuta tu codigo. Es el analogo Windows del `path-hijacking` de Linux.

El orden de busqueda de DLLs incluye el directorio del ejecutable y otros; si la app no fija rutas y falta una DLL, el hueco es explotable. Tu DLL (con su DllMain) ejecuta el payload en el contexto del proceso, que puede ser SYSTEM.
...
Cuando un servicio o app privilegiada tiene una DLL ausente o cargable desde un directorio escribible. ProcMon (filtrando NAME NOT FOUND en .dll) lo revela.
...
Filtra ProcMon por NAME NOT FOUND y Path ends with .dll mientras el proceso privilegiado corre.

Evidencia esperada: Una ruta de DLL no encontrada en un directorio donde puedes escribir; tras colocar tu DLL, ejecucion en el contexto del proceso.
...
```text
procmon.exe (filtro: Result is NAME NOT FOUND, Path ends with .dll)
```

**Objetivo y contexto:** ProcMon monitoriza en vivo que ficheros abre cada proceso. Filtrando DLLs no encontradas ves exactamente que hueco puedes rellenar con tu DLL.

**Resultado esperado:** Rutas de DLLs que un proceso busca sin exito. Si alguna esta en un directorio escribible por ti y el proceso es privilegiado, ese es tu vector.

### Capa 2: prueba documentada

```text
x86_64-w64-mingw32-gcc pe.c -shared -o legit.dll
```

**Objetivo y contexto:** Compila (desde Kali) una DLL cuyo DllMain ejecuta tu payload al cargarse. El nombre debe coincidir con la DLL que el proceso busca.

**Resultado esperado:** El fichero legit.dll. Colocalo en el directorio donde el proceso la busca; al reiniciarse/cargarla, ejecuta tu codigo en su contexto (idealmente SYSTEM).

## Anatomía de los payloads

La primera prueba es `procmon.exe (filtro: Result is NAME NOT FOUND, Path ends with .dll)`. Separa sus delimitadores, operadores, opciones, operandos y variables; algunos elementos no estarán presentes según el contexto. Las comillas, barras y separadores pertenecen a una capa concreta. Usa la explicación de cada capa para determinar qué símbolo altera sintaxis y cuál transporta datos.
...
Una ruta de DLL no encontrada en un directorio donde puedes escribir; tras colocar tu DLL, ejecucion en el contexto del proceso.
...
Una app privilegiada que busca una DLL en un directorio escribible carga la tuya: es el PATH hijacking de Windows. La técnica queda confirmada solo cuando una prueba mínima produce la evidencia prevista y descarta explicaciones alternativas.
...
- [Concepto fuente de THM Fieldbook](../../tools/concepts.py) (`dll-hijacking`)
