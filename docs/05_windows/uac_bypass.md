...
El Control de Cuentas de Usuario (UAC) hace que, aunque tu usuario sea administrador, ejecutes con un token de integridad media hasta que confirmas la elevacion. Un bypass de UAC aprovecha binarios de Windows que auto-elevan sin pedir consentimiento para lanzar un proceso con integridad alta (admin completo) sin el prompt.

Ciertos ejecutables firmados de Microsoft se auto-elevan y leen configuracion del registro de usuario; si secuestras esa clave (fodhelper, eventvwr), Windows lanza tu comando elevado. No es cruzar de usuario a admin, sino de 'admin a medias' a 'admin completo'.
...
Cuando whoami /groups muestra que estas en Administrators pero con integridad media (no puedes tocar recursos protegidos aun). No aplica si ya eres SYSTEM o integridad alta.
...
whoami /groups | findstr Label para ver tu nivel de integridad actual.

Evidencia esperada: 'Mandatory Label\Medium'; tras el bypass, un proceso con integridad High ejecutando tu comando.
...
**Objetivo y contexto:** Muestra tu nivel de integridad (Mandatory Label). Distingue si necesitas bypass de UAC (Medium) o si ya tienes admin completo (High).

**Resultado esperado:** 'Mandatory Label\Medium' = necesitas UAC bypass. 'High' = ya elevado, no hace falta. Junto con el grupo Administrators define si aplica.
...
**Objetivo y contexto:** Prepara el secuestro de fodhelper: al ejecutarse (auto-elevado), lee esta clave de usuario y lanza tu comando con integridad alta, sin prompt de UAC.

**Resultado esperado:** Tras esto, ejecutar fodhelper.exe abre un cmd elevado (integridad High). Ajusta la tecnica/clave a la build de Windows; limpia la clave despues.

## Anatomía de los payloads

La primera prueba es `whoami /groups | findstr /i "Label"`. Separa sus delimitadores, operadores, opciones, operandos y variables; algunos elementos no estarán presentes según el contexto. Las comillas, barras y separadores pertenecen a una capa concreta. Usa la explicación de cada capa para determinar qué símbolo altera sintaxis y cuál transporta datos.
...
'Mandatory Label\Medium'; tras el bypass, un proceso con integridad High ejecutando tu comando.
...
Ya eres admin pero con token limitado; un bypass de UAC eleva a integridad alta sin el prompt de consentimiento. La técnica queda confirmada solo cuando una prueba mínima produce la evidencia prevista y descarta explicaciones alternativas.
...
- [Concepto fuente de THM Fieldbook](../../tools/concepts.py) (`uac-bypass`)
