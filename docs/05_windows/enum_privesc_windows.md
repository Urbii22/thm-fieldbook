---
titulo: "Enumeracion para privesc en Windows"
...
fuentes_internas:
  - ../../tools/concepts.py#enum-privesc-windows
fuentes_externas:
...
# Enumeracion para privesc en Windows
...
Enumerar para privesc en Windows es recorrer sus vectores tipicos en orden: privilegios de token, grupos, servicios mal configurados, tareas programadas, credenciales guardadas y parches faltantes. WinPEAS lo automatiza; entender cada hallazgo es lo que lo convierte en escalada. Paso previo del `winprivesc-modelo`.

Windows esconde muchos vectores en configuracion (servicios, registro, permisos NTFS) que a mano son tediosos. WinPEAS y las consultas manuales garantizan cobertura, pero dan falsos positivos: hay que validar cada pista.
...
Nada mas conseguir una shell en Windows. Corre primero whoami /priv y whoami /groups, luego WinPEAS para lo exhaustivo.
...
whoami /priv y whoami /groups primero; despues winPEAS para el resto.

Evidencia esperada: Un privilegio de token Enabled, o un hallazgo resaltado en winPEAS (servicio, credencial, AlwaysInstallElevated).
...
**Objetivo y contexto:** Lista tus privilegios de token, el vector mas rapido en Windows. Varios (SeImpersonate, SeBackup, SeRestore, SeDebug) llevan a SYSTEM casi directo.

**Resultado esperado:** Privilegios y si estan Enabled. SeImpersonate -> `token-impersonation`. SeBackup/SeRestore -> `sebackup-serestore`. Anota los que esten habilitados.
...
**Objetivo y contexto:** Muestra version de Windows y hotfixes instalados. Comparar los parches con los CVE conocidos revela si hay un exploit de kernel aplicable.

**Resultado esperado:** OS version, build y lista de KB (parches). Pocos KB en un Windows viejo -> candidato a exploit de kernel; pasalo por Watson/wesng.

## Anatomía de los payloads

La primera prueba es `whoami /priv`. Separa sus delimitadores, operadores, opciones, operandos y variables; algunos elementos no estarán presentes según el contexto. Las comillas, barras y separadores pertenecen a una capa concreta. Usa la explicación de cada capa para determinar qué símbolo altera sintaxis y cuál transporta datos.
...
Un privilegio de token Enabled, o un hallazgo resaltado en winPEAS (servicio, credencial, AlwaysInstallElevated).
...
Igual que en Linux pero con otros checks: privilegios de token, servicios, credenciales guardadas y parches faltantes. La técnica queda confirmada solo cuando una prueba mínima produce la evidencia prevista y descarta explicaciones alternativas.
...
- [Concepto fuente de THM Fieldbook](../../tools/concepts.py) (`enum-privesc-windows`)
