---
titulo: "Docker y escapes de contenedor"
categoria: 04_linux
...
fuentes_internas:
  - ../../tools/concepts.py#docker-container-escape
fuentes_externas:
...
# Docker y escapes de contenedor
...
Un contenedor comparte kernel con el host. El acceso al socket Docker, un contenedor privilegiado o un bind mount del filesystem del host puede convertir una shell en el contenedor en control del host.

El daemon Docker actua con privilegios del host y los montajes trasladan esos permisos al proceso dentro del contenedor.
...
Tras obtener shell en un contenedor o cuando id/groups muestra docker y el socket /var/run/docker.sock es accesible.
...
Identifica socket, capacidades, mounts y privilegios; confirma impacto solo con un fichero de prueba no destructivo.

Evidencia esperada: Sabes si el contenedor es una frontera real o una via de escritura/lectura sobre el host.
...
```text
id; ls -l /var/run/docker.sock; capsh --print 2>/dev/null
```

**Objetivo y contexto:** Recoge grupo, socket y capacidades antes de intentar cualquier accion.

**Resultado esperado:** Permisos del socket y capabilities efectivas.

### Capa 2: prueba documentada

```text
docker ps --format '{{.ID}} {{.Image}} {{.Mounts}}'
```

**Objetivo y contexto:** Comprueba si puedes hablar con el daemon y que montajes existen.

**Resultado esperado:** Contenedores y montajes visibles al usuario actual.

## Anatomía de los payloads

La primera prueba es `id; ls -l /var/run/docker.sock; capsh --print 2>/dev/null`. Separa sus delimitadores, operadores, opciones, operandos y variables; algunos elementos no estarán presentes según el contexto. Las comillas, barras y separadores pertenecen a una capa concreta. Usa la explicación de cada capa para determinar qué símbolo altera sintaxis y cuál transporta datos.
...
Sabes si el contenedor es una frontera real o una via de escritura/lectura sobre el host.
...
- Identifica runtime y usuario.
- Revisa socket, capabilities y mounts.
- Prueba acceso a un recurso de laboratorio.
- Registra la remediacion: retirar socket, privilegios y mounts innecesarios.

Amplía únicamente dentro del laboratorio y cuando cada salto aporte una capacidad nueva demostrable.

## Errores frecuentes

- El proceso esta en una VM sin Docker accesible.
- El socket no existe y no hay privilegios especiales ni montajes del host.
...
Distinguir aislamiento normal de montajes, sockets y privilegios que permiten alcanzar el host. La técnica queda confirmada solo cuando una prueba mínima produce la evidencia prevista y descarta explicaciones alternativas.
...
- [Concepto fuente de THM Fieldbook](../../tools/concepts.py) (`docker-container-escape`)
