---
titulo: "Ficheros sensibles escribibles"
categoria: 04_linux
...
fuentes_internas:
  - ../../tools/concepts.py#writable-sensitive-files
fuentes_externas:
...
# Ficheros sensibles escribibles
...
Cuando la enumeracion (`enum-privesc-linux`) marca /etc/passwd, /etc/shadow o /etc/sudoers como escribibles por ti (ls -l o el check de linpeas).
...
ls -l /etc/passwd /etc/shadow /etc/sudoers y busca un permiso w para ti.

Evidencia esperada: Un 'w' en los permisos de alguno de esos ficheros para tu usuario o grupo.
...
**Objetivo y contexto:** Comprueba de un vistazo si tienes escritura en los ficheros que definen identidad y privilegios. Es un check barato con recompensa enorme.

**Resultado esperado:** Los permisos. Una 'w' para tu usuario/grupo en cualquiera de ellos es escalada directa. Normalmente solo root escribe; si no, hay mala config.

### Capa 2: prueba documentada

```text
openssl passwd -1 -salt xyz Passw0rd
```

**Objetivo y contexto:** Genera un hash valido para /etc/passwd. Si passwd es escribible, anades una linea 'hacker:HASH:0:0::/root:/bin/bash' y creas un root con password conocida.

**Resultado esperado:** El hash a pegar en el segundo campo. Tras anadir la linea, 'su hacker' con Passw0rd te da uid=0. -1 usa MD5crypt, compatible con /etc/passwd.

## Anatomía de los payloads

La primera prueba es `ls -l /etc/passwd /etc/shadow /etc/sudoers`. Separa sus delimitadores, operadores, opciones, operandos y variables; algunos elementos no estarán presentes según el contexto. Las comillas, barras y separadores pertenecen a una capa concreta. Usa la explicación de cada capa para determinar qué símbolo altera sintaxis y cuál transporta datos.
...
Un 'w' en los permisos de alguno de esos ficheros para tu usuario o grupo.
...
Si puedes escribir /etc/passwd, /etc/shadow o /etc/sudoers, te conviertes en root editando una linea. La técnica queda confirmada solo cuando una prueba mínima produce la evidencia prevista y descarta explicaciones alternativas.
...
- [Concepto fuente de THM Fieldbook](../../tools/concepts.py) (`writable-sensitive-files`)
