---
titulo: "BloodHound: el grafo de permisos"
...
# BloodHound: el grafo de permisos
...
BloodHound recolecta usuarios, grupos, equipos, sesiones y ACLs del dominio y los representa como un grafo. En lugar de adivinar que permiso abusar, ves la ruta concreta: quien puede resetear la password de quien, que grupo tiene GenericAll sobre otro, donde hay sesiones de admin que robar.

En un dominio real las relaciones de permisos son miles y a mano es imposible. El grafo revela caminos no obvios (encadenar tres permisos mediocres para llegar a DA) que definen el ataque en AD (`ad-modelo`).
...
En cuanto tengas UNA credencial valida. Es lo que haces cuando el roasting (`asrep-kerberoast`) no basto o para planificar el salto final a Domain Admin.
...
Corre bloodhound-python con la credencial y confirma que el .zip se genera sin errores de autenticacion.

Evidencia esperada: El .zip se importa en la GUI y aparece tu usuario en el grafo; 'Shortest paths to Domain Admins' muestra al menos una ruta.
...
**Objetivo y contexto:** Recolecta todo el grafo del dominio con una sola credencial, sin necesidad de pisar una maquina Windows. -c all junta toda la informacion; --zip lo deja listo para importar.

**Resultado esperado:** Un .zip para arrastrar a la GUI de BloodHound. Alli marca tu usuario como owned y lanza la consulta de rutas a Domain Admins.

## Anatomía de los payloads

La primera prueba es `bloodhound-python -d $DOMAIN -u $USER -p $PASS -c all -ns $IP --zip`. Separa sus delimitadores, operadores, opciones, operandos y variables; algunos elementos no estarán presentes según el contexto. Las comillas, barras y separadores pertenecen a una capa concreta. Usa la explicación de cada capa para determinar qué símbolo altera sintaxis y cuál transporta datos.
...
El .zip se importa en la GUI y aparece tu usuario en el grafo; 'Shortest paths to Domain Admins' muestra al menos una ruta.
...
Mapea el dominio como un grafo y te muestra la ruta mas corta de tu usuario a Domain Admin. La técnica queda confirmada solo cuando una prueba mínima produce la evidencia prevista y descarta explicaciones alternativas.
...
- [Concepto fuente de THM Fieldbook](../../tools/concepts.py) (`bloodhound`)
