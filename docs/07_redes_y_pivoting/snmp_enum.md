...
SNMP (puerto 161/UDP) es un protocolo de gestion y monitorizacion de dispositivos de red. Se autentica con una 'community string' en vez de usuario/password; si el dispositivo sigue usando la que trae por defecto ('public' para lectura), cualquiera que la conozca puede consultar una cantidad enorme de informacion del sistema: procesos en ejecucion (con sus argumentos, que a veces llevan credenciales), interfaces de red, software instalado, y en Windows a veces hasta usuarios locales.

La community string por defecto es publica y conocida (esta en cualquier manual); si el administrador no la cambia, SNMP queda abierto a cualquiera que sepa probarla. No es un fallo del protocolo, es no cambiar una credencial por defecto, igual que `smb-enum` con sesiones nulas.
...
Cuando el recon muestra 161/UDP abierto (nmap -sU lo detecta, un escaneo TCP normal NO lo vera). Muy rentable en maquinas de red/infraestructura.
...
Consulta con snmpwalk usando la community 'public' (la mas comun por defecto).

Evidencia esperada: Una respuesta con datos del sistema (nombre de host, procesos, interfaces) confirma que la community funciona. Timeout repetido sugiere filtrado UDP o community incorrecta (dificil distinguir sin mas pruebas).
...
**Objetivo y contexto:** Vuelca toda la informacion accesible via SNMPv2 con la community 'public', la que casi siempre viene por defecto si nadie la cambio.

**Resultado esperado:** Cientos de lineas con datos del sistema. Busca especificamente procesos (hrSWRunParameters) y su linea de comandos completa: ahi caen credenciales a menudo.

## Anatomía de los payloads

La primera prueba es `snmpwalk -v2c -c public $IP`. Separa sus delimitadores, operadores, opciones, operandos y variables; algunos elementos no estarán presentes según el contexto. Las comillas, barras y separadores pertenecen a una capa concreta. Usa la explicación de cada capa para determinar qué símbolo altera sintaxis y cuál transporta datos.
...
Una respuesta con datos del sistema (nombre de host, procesos, interfaces) confirma que la community funciona. Timeout repetido sugiere filtrado UDP o community incorrecta (dificil distinguir sin mas pruebas).
...
SNMP con la community string 'public' filtra procesos, usuarios y software instalado sin necesitar credenciales reales. La técnica queda confirmada solo cuando una prueba mínima produce la evidencia prevista y descarta explicaciones alternativas.
...
- [Concepto fuente de THM Fieldbook](../../tools/concepts.py) (`snmp-enum`)
