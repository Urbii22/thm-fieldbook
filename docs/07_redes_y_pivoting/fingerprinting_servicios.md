---
titulo: "Fingerprinting antes que el nombre del puerto"
...
fuentes_internas:
  - ../../tools/concepts.py#fingerprinting-servicios
fuentes_externas:
...
# Fingerprinting antes que el nombre del puerto
...
IANA y Nmap pueden asociar un puerto a un nombre habitual, pero cualquier proceso puede escuchar ahi. Fingerprinting es identificar lo que realmente responde mediante handshake, cabeceras, banner, TLS y deteccion de version.

Asumir que 5050 es 'mmcc' puede hacer que ignores una web Flask o Werkzeug. El servicio real determina tus siguientes pruebas: HTTP se enumera con rutas, formularios y cabeceras; SSH con usuarios y credenciales; no con el nombre historico del puerto.
...
Siempre que encuentres un puerto no estandar, un banner incompleto o una asociacion de Nmap que no encaje con la respuesta observada.
...
- -sV identifica un producto/framework distinto de la asociacion por puerto.
...
Nmap -sV, curl o una conexion de protocolo devuelve una respuesta coherente con el servicio real.

Evidencia esperada: Tienes protocolo, producto y version para decidir la enumeracion y buscar vulnerabilidades aplicables.
...
```text
nmap -sC -sV -Pn -p <PUERTO> $IP
```

**Objetivo y contexto:** Confirma que habla realmente el puerto y recoge scripts/banners utiles.

**Resultado esperado:** Servicio, producto, version y pistas como titulos HTTP o certificados.
...
**Objetivo y contexto:** Una comprobacion HTTP directa separa una aplicacion web real de una etiqueta de puerto equivocada.

**Resultado esperado:** Status, cabeceras y posiblemente framework o rutas a enumerar.

## Anatomía de los payloads

La primera prueba es `nmap -sC -sV -Pn -p <PUERTO> $IP`. Separa sus delimitadores, operadores, opciones, operandos y variables; algunos elementos no estarán presentes según el contexto. Las comillas, barras y separadores pertenecen a una capa concreta. Usa la explicación de cada capa para determinar qué símbolo altera sintaxis y cuál transporta datos.
...
Tienes protocolo, producto y version para decidir la enumeracion y buscar vulnerabilidades aplicables.
...
- Registra el puerto abierto sin asumir su funcion.
- Ejecuta deteccion de version y observa la respuesta de protocolo.
- Enumera segun el servicio confirmado y no segun el alias del puerto.
- Guarda producto y version exactos como evidencia para el siguiente paso.

Amplía únicamente dentro del laboratorio y cuando cada salto aporte una capacidad nueva demostrable.

## Errores frecuentes

- No sustituyas una identificacion de version fiable por el nombre del puerto.
- No concluyas que un servicio esta ausente solo porque no usa su puerto por defecto.
...
El nombre asignado a un puerto es una pista, no una identidad: confirma protocolo, producto y version antes de elegir la enumeracion. La técnica queda confirmada solo cuando una prueba mínima produce la evidencia prevista y descarta explicaciones alternativas.
...
- [Concepto fuente de THM Fieldbook](../../tools/concepts.py) (`fingerprinting-servicios`)
