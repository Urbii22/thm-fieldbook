---
titulo: "Trafico de red, poisoning y relay"
...
fuentes_internas:
  - ../../tools/concepts.py#network-traffic-mitm
fuentes_externas:
...
# Trafico de red, poisoning y relay
...
Observar trafico describe protocolos y flujos; poisoning responde a resoluciones inseguras para provocar autenticacion; relay reenvia esa autenticacion a otro servicio. Son niveles de intervencion y riesgo diferentes.

LLMNR/NBT-NS y NTLM pueden exponer hashes NetNTLM o permitir relay cuando la firma SMB no es obligatoria. La segmentacion y los controles de firma determinan si el ataque atraviesa la red.
...
En un segmento de laboratorio donde aparecen broadcasts de resolucion, autenticacion NTLM y objetivos con SMB signing desactivado.
...
Separa evidencia pasiva, hash capturado y autenticacion reenviada; no presentes una captura como si ya implicara ejecucion.

Evidencia esperada: Sabes si existe solo exposicion de credenciales, posibilidad de crackeo offline o una ruta de relay con impacto lateral.
...
**Objetivo y contexto:** Escucha y responde en la interfaz autorizada para capturar autenticacion de laboratorio.

**Resultado esperado:** Hash NetNTLMv2 con usuario y host de origen.
...
**Objetivo y contexto:** Prueba relay solo contra objetivos previamente validados sin firma SMB obligatoria.

**Resultado esperado:** Autenticacion aceptada o rechazo que confirma el control defensivo.

## Anatomía de los payloads

La primera prueba es `sudo responder -I tun0 -wv`. Separa sus delimitadores, operadores, opciones, operandos y variables; algunos elementos no estarán presentes según el contexto. Las comillas, barras y separadores pertenecen a una capa concreta. Usa la explicación de cada capa para determinar qué símbolo altera sintaxis y cuál transporta datos.
...
Sabes si existe solo exposicion de credenciales, posibilidad de crackeo offline o una ruta de relay con impacto lateral.
...
- Identifica interfaz y alcance.
- Observa antes de responder.
- Captura una autenticacion de laboratorio.
- Comprueba signing en objetivos.
- Solo entonces evalua relay y documenta impacto.

Amplía únicamente dentro del laboratorio y cuando cada salto aporte una capacidad nueva demostrable.

## Errores frecuentes

- No estas en el mismo segmento ni tienes un pivot con visibilidad del broadcast.
- SMB signing es obligatorio en los objetivos de relay.
- El alcance no autoriza poisoning o manipulacion de trafico.
...
Distinguir observacion pasiva, captura de autenticacion y relay activo dentro del mismo segmento. La técnica queda confirmada solo cuando una prueba mínima produce la evidencia prevista y descarta explicaciones alternativas.
...
- [Concepto fuente de THM Fieldbook](../../tools/concepts.py) (`network-traffic-mitm`)
