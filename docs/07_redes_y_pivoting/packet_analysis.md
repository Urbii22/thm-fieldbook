---
titulo: "Captura y analisis de paquetes"
...
fuentes_internas:
  - ../../tools/concepts.py#packet-analysis
fuentes_externas:
...
Una captura PCAP conserva paquetes observados en una interfaz. Los filtros de captura reducen que se guarda; los filtros de visualizacion seleccionan despues que analizar. Seguir un stream reconstruye la conversacion de una sesion TCP.

Mirar paquetes aislados produce conclusiones pobres. Relacionar DNS, establecimiento TCP, protocolo de aplicacion, autenticacion y errores permite identificar hosts, servicios y datos expuestos con evidencia temporal.
...
Cuando tienes permiso y visibilidad de una interfaz, una PCAP de una room o necesitas entender por que dos sistemas no se comunican.
...
Filtra por protocolo o host, sigue el stream relevante y relaciona cada conclusion con paquetes y marcas de tiempo.

Evidencia esperada: Una secuencia explicable de quien inicio la conexion, que protocolo uso, que respondio el destino y donde fallo.
...
```text
tshark -r capture.pcap -Y 'dns or http or kerberos or smb2'
```

**Objetivo y contexto:** Aplica un filtro de visualizacion reproducible sobre una captura ya obtenida sin modificar el trafico.

**Resultado esperado:** Paquetes relevantes con tiempo, origen, destino, protocolo y resumen.

## Anatomía de los payloads

La primera prueba es `tshark -r capture.pcap -Y 'dns or http or kerberos or smb2'`. Separa sus delimitadores, operadores, opciones, operandos y variables; algunos elementos no estarán presentes según el contexto. Las comillas, barras y separadores pertenecen a una capa concreta. Usa la explicación de cada capa para determinar qué símbolo altera sintaxis y cuál transporta datos.
...
Una secuencia explicable de quien inicio la conexion, que protocolo uso, que respondio el destino y donde fallo.
...
- Confirma interfaz y tiempo.
- Captura con el menor filtro necesario.
- Aplica filtros de visualizacion.
- Sigue la conversacion.
- Registra paquetes que sustentan la conclusion.

Amplía únicamente dentro del laboratorio y cuando cada salto aporte una capacidad nueva demostrable.

## Errores frecuentes

- No ver trafico no demuestra que el servicio no exista: puede faltar visibilidad, interfaz o ventana temporal.
- TLS protege el contenido aunque metadatos como IP, puerto, SNI y tiempos sigan siendo visibles.
...
Separar captura, filtrado e interpretacion para reconstruir conversaciones de red sin intervenir en ellas. La técnica queda confirmada solo cuando una prueba mínima produce la evidencia prevista y descarta explicaciones alternativas.
...
- [Concepto fuente de THM Fieldbook](../../tools/concepts.py) (`packet-analysis`)
