---
titulo: "Segmentacion, rutas y firewalls"
...
# Segmentacion, rutas y firewalls
...
La segmentacion separa redes y limita sus flujos mediante rutas, ACL y firewalls. Alcanzable describe que existe un camino; permitido describe que los controles dejan pasar protocolo, puerto y direccion concretos.

Un host comprometido puede tener rutas que Kali no posee. Tambien puede aceptar conexiones salientes pero bloquear entrantes, motivo por el que un tunel reverse funciona donde uno directo falla.
...
Cuando aparecen varias interfaces, subredes internas, puertos filtered, respuestas asimetricas o servicios accesibles solo desde otro host.
...
Compara rutas e interfaces desde ambos extremos y prueba un unico host/puerto conocido antes de lanzar enumeracion amplia.

Evidencia esperada: Un mapa de origen, destino, ruta, direccion permitida y punto de filtrado que justifica si necesitas pivot o port forwarding.
...
**Objetivo y contexto:** Muestra que redes conoce el host actual y por que interfaz o gateway intentara alcanzarlas.

**Resultado esperado:** Prefijos, gateways e interfaces que revelan redes internas y posibles pivots.

## Anatomía de los payloads

La primera prueba es `ip route`. Separa sus delimitadores, operadores, opciones, operandos y variables; algunos elementos no estarán presentes según el contexto. Las comillas, barras y separadores pertenecen a una capa concreta. Usa la explicación de cada capa para determinar qué símbolo altera sintaxis y cuál transporta datos.
...
Un mapa de origen, destino, ruta, direccion permitida y punto de filtrado que justifica si necesitas pivot o port forwarding.
...
- Dibuja redes por host.
- Compara las rutas.
- Prueba un puerto conocido.
- Identifica direccion y control.
- Elige tunel o forwarding minimo.

Amplía únicamente dentro del laboratorio y cuando cada salto aporte una capacidad nueva demostrable.

## Errores frecuentes

- Closed indica que el host respondio pero no hay servicio; filtered suele indicar ausencia de respuesta o filtrado y exige otra hipotesis.
- Un tunel no concede permisos de aplicacion: solo cambia la conectividad.
...
Distinguir que redes conoce cada host, que camino sigue el trafico y que control permite o bloquea cada direccion. La técnica queda confirmada solo cuando una prueba mínima produce la evidencia prevista y descarta explicaciones alternativas.
...
- [Concepto fuente de THM Fieldbook](../../tools/concepts.py) (`network-segmentation-firewalls`)
