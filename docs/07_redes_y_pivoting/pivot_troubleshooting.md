---
titulo: "Diagnóstico de pivoting"
...
fuentes_internas:
  - ../../tools/concepts.py#pivot-troubleshooting
fuentes_externas:
...
Pivoting depende de que la ruta, el proceso de tunel y el proxy conozcan correctamente cada red.

Un tunel activo no garantiza que el trafico use la interfaz o el puerto correctos.
...
Cuando ves la maquina pivote pero un servicio interno da timeout o connection refused.
...
Comprueba interfaz interna, ruta, escucha del proxy y conexion desde el punto correcto.

Evidencia esperada: Sabes si falla el tunel, el proxy, la ruta o el servicio objetivo.
...
**Objetivo y contexto:** Revela interfaces y redes que realmente ve el pivote.

**Resultado esperado:** Subred interna, gateway y rutas disponibles.
...
**Objetivo y contexto:** Confirma que el servicio de tunel o proxy escucha donde esperas.

**Resultado esperado:** Puerto, interfaz y proceso propietario.

### Capa 3: prueba documentada

```text
proxychains -q nmap -Pn -p <PUERTO> <IP_INTERNA>
```

**Objetivo y contexto:** Valida el camino completo hacia un servicio interno.

**Resultado esperado:** open, filtered o timeout a traves del proxy.

## Anatomía de los payloads

La primera prueba es `ip addr; ip route`. Separa sus delimitadores, operadores, opciones, operandos y variables; algunos elementos no estarán presentes según el contexto. Las comillas, barras y separadores pertenecen a una capa concreta. Usa la explicación de cada capa para determinar qué símbolo altera sintaxis y cuál transporta datos.
...
Sabes si falla el tunel, el proxy, la ruta o el servicio objetivo.
...
- Inspecciona interfaces.
- Comprueba rutas y listener.
- Prueba desde el pivote y desde proxychains.

Amplía únicamente dentro del laboratorio y cuando cada salto aporte una capacidad nueva demostrable.

## Errores frecuentes

- No aplica si el objetivo interno es directamente alcanzable desde tu maquina.
...
Comprobar rutas, listeners y alcance cuando el tunel existe pero no llega al servicio interno. La técnica queda confirmada solo cuando una prueba mínima produce la evidencia prevista y descarta explicaciones alternativas.
...
- [Concepto fuente de THM Fieldbook](../../tools/concepts.py) (`pivot-troubleshooting`)
