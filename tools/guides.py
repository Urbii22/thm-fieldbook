        ...
        "steps": [
            {
                ...
                "idea": "Empieza por documentacion y metodos observables. El objetivo no es lanzar payloads, sino saber que recursos existen y que entrada controla cada uno.",
                "commands": [
                    {"cmd": "curl -sS $URL/swagger.json", "why": "Busca un contrato OpenAPI publicado.", "out": "Rutas, metodos y esquemas disponibles."},
                    {"cmd": "curl -sS -i -X OPTIONS \"$URL/api/resource\"", "why": "Comprueba metodos aceptados por un recurso.", "out": "Cabecera Allow y comportamiento del endpoint."},
                ],
                "look": "Versiones de API, endpoints administrativos, parametros identificadores y respuestas JSON.",
                "decide": "Si falta documentacion, continua con el trafico de la SPA y compara respuestas por metodo.",
            },
            {
                "title": "Fija el contexto de sesion",
                "idea": "Separa siempre peticiones publicas, autenticadas y de otra cuenta. Esa comparacion evita confundir un fallo de autenticacion con uno de autorizacion.",
                "commands": [
                    ...
                    {"cmd": "curl -sS -i \"$URL/api/me\"", "why": "Establece la respuesta sin credenciales.", "out": "401/403 o respuesta publica."},
                ],
                "look": "Cookies, cabeceras Bearer, expiracion, refresh y diferencias entre 401 y 403.",
                "decide": "Si el token no cambia el contexto, revisa formato, expiracion y endpoint de login antes de probar permisos.",
            },
            {
                "title": "Prueba autorizacion por objeto",
                "idea": "Con dos cuentas de laboratorio, conserva el mismo endpoint y cambia solo el identificador del objeto. Un 200 sobre un objeto ajeno es mas importante que una respuesta generica de login.",
                "commands": [
                    {"cmd": "curl -sS \"$URL/api/users/1001\" -H \"Authorization: Bearer $TOKEN_A\"", "why": "Lee un objeto propio como cuenta A.", "out": "Campos y codigo de respuesta de referencia."},
                ...
                "look": "Cambios de status, tamano y campos sensibles al variar solo el ID.",
            ...
            },
            {
                ...
                "decide": "Prioriza una validacion reproducible de permisos; deja las pruebas de alteracion para un laboratorio controlado.",
            },
            {
                "title": "Cierra con evidencia y siguiente paso",
                "idea": "Un hallazgo util incluye peticion, identidad, respuesta esperada y respuesta observada. Asi puedes volver atras, comparar otra cuenta y decidir si toca corregir el alcance o pasar a otra fase.",
                "commands": [
                    {"cmd": "curl -sS -i \"$URL/api/resource\" -H \"Authorization: Bearer $TOKEN\"", "why": "Guarda una respuesta completa como evidencia minima.", "out": "Status, cabeceras y cuerpo para comparar en el writeup."},
                ],
                "look": "Request reproducible, contexto de cuenta, objeto afectado y limite del impacto.",
                "decide": "Si el control funciona, pasa al siguiente endpoint del inventario; si falla, conserva el caso minimo y enlazalo con [[idor-bola]].",
            },
        ],
    },
    {
